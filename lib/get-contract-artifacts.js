...
const getFullPathToArtifact = memoizeWith(identity, _ =>
  path.resolve(__dirname, '../artifacts/contracts/pToken.sol/PToken.json')
)
...
const getArtifact = memoizeWith(identity, _path =>
  new Promise((resolve, reject) => {
    ...
    exists
      ...
      : reject(new Error('pToken artifact does not exist! Run `npx hardhat compile` to compile contracts!'))
  })
)
